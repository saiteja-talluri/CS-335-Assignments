...
def kmeans_iteration_one(data, centroids, distance):
	...
	for i in range(len(data)):
		...
		for j in range(len(centroids)):
			cur_dist = distance(data[i],centroids[j])
			if cur_dist < dist_min:
				min_index = j
				dist_min = cur_dist
		holder[min_index].append(data[i])

	new_centroids = []

	for i in range(len(holder)):
		tuple_sum = [0] * len(data[0])

		if len(holder[i]) != 0:
			for j in range(len(holder[i])):
				counter = map(lambda x,y:x+y, tuple_sum, list(holder[i][j]))
				tuple_sum = deepcopy(counter)
			tuple_sum = deepcopy(tuple(map(lambda x: (x*1.0)/len(holder[i]), tuple_sum)))
			new_centroids.append(tuple_sum)
		else:
...
def performance_SSE(data, centroids, distance):
	...
	for i in range(len(holder)):
		for j in range(len(holder[i])):
			sse += (distance(holder[i][j],centroids[i]))**2
...
def initialization_kmeansplusplus(data, distance, k):
	...
	# Intially the list of centroids is empty
	centroids = []
	centroid_index = []

	# In every iteration we add a new centroid
	for i in range(k):
		if len(centroids) == 0: # Base case when length is zero
			init = 0
			while 1:
				rand = random.random()
				if(rand != 1):
					init = int(rand*len(data))
					break
			centroids.append(data[init]) # At the start there are no centroids so randomly sample a data point and add it to centroids
			centroid_index.append(init)  # Adding the corresponding index of the centroid to a list

		else:
			distribution = [] 
			data_index = []
			for i in range(len(data)):	# Looping over the data points to check for the nearest centroid
				if i not in centroid_index: # Ensuring that the point is not  a centroid
					dist_min  = 100**10 
					min_index = 0
					for j in range(len(centroids)):	# Looping over the centroids for a given data point
						cur_dist = distance(data[i],centroids[j])
						if (cur_dist < dist_min):
							min_index = j
							dist_min = cur_dist
					distribution.append(dist_min**2) # Appended square of nearest distance to the distribution
					data_index.append(i) # Appended the index of the point in the original data
			
			total = sum(distribution)
			normalized_distribution = [(x*1.0)/total for x in distribution] # Normalized the distribution

			probe = random.random() # Random probe
			cdf = normalized_distribution[0]

			# Basically implementing the sampling as per the distribution using random probe and cdfs
			is_found = 0
			for i in range(1,len(normalized_distribution)):
				if (probe < cdf): # Checking the probe is less than the cdf of the point
					centroids.append(data[data_index[i-1]])
					centroid_index.append(data_index[i-1])
					is_found = 1
					break
				else :
					cdf += normalized_distribution[i]

			if is_found == 0:
				centroids.append(data[data_index[len(normalized_distribution)-1]])
				centroid_index.append(data_index[len(normalized_distribution)-1])
...
def distance_manhattan(p1, p2):
	...
	manhat_dist = 0
	for i in range(len(p1)) :
		manhat_dist += abs(p1[i] - p2[i])
	distance = manhat_dist
...
def kmedians_iteration_one(data, centroids, distance):
	...
	def median(arr):
		...
		ans = 0
		if  len(arr)%2 == 0:
			ans = 0.5*(arr[len(arr)/2]+arr[(len(arr)/2)-1])
		else :
			ans = arr[(len(arr)-1)/2]
		return ans
	...
	for i in range(len(holder)):
		if len(holder[i]) != 0:
			...
			for k in range(len(data[0])):
				index_centroid = []
				for j in range(len(holder[i])):
					index_centroid.append(holder[i][j][k])
				curr_centroid.append(median(index_centroid))
			new_centroids.append(deepcopy(tuple(curr_centroid)))
...
def performance_L1(data, centroids, distance):
	...
	for i in range(len(holder)):
		for j in range(len(holder[i])):
			l1_error += distance(holder[i][j],centroids[i])
