...
class SudokuSearchProblem:
    ...
    def getSuccessors(self, state):
        ...
        successors = []
        ## YOUR CODE HERE
        min_opt = min([len(state[s]) for s in squares if len(state[s]) > 1])
        for s in state:
            if(len(state[s]) == min_opt):
                for d in state[s]:
                    u = assign(state.copy(),s,d)
                    if (u != False):
                        successors.append((u,(s,d),1))
                break
        return successors
...
if __name__ == '__main__':
    ...
    prob2  = '003020600900305001001806400008102900700000008006708200002609500800203009005010300'
    # display(parse_grid(prob2))