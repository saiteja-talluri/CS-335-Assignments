...
def generate2optNeighbours(tour):
    ...
    for u in range(0, cities - 2):
        for v in range(u + 2,cities):
            if u == 0 and v == cities - 1:
                continue
            opt_tour = tour.copy()
            opt_tour[u+1:v+1] = tour[u+1:v+1][::-1]
            all_possible_neighbours.append(opt_tour)
...
def generate3optNeighbours(tour):
    ...
    "*** YOUR CODE HERE ***"
    for u in range(0, cities - 4):
        for v in range(u + 2,cities - 2):
            for w in range(v + 2, cities):
                if u == 0 and w == cities - 1:
                    continue
                opt_tour = tour.copy()
                opt_tour[u+1:v+1] = tour[u+1:v+1][::-1]
                opt_tour[v+1:w+1] = tour[v+1:w+1][::-1]
                all_possible_neighbours.append(opt_tour)    

                opt_tour = tour.copy()
                opt_tour[u+1:w-v+u+1] = tour[v+1:w+1]
                opt_tour[w-v+u+1:w+1] = tour[u+1:v+1]
                all_possible_neighbours.append(opt_tour)

                opt_tour = tour.copy()
                opt_tour[u+1:w-v+u+1] = tour[v+1:w+1]
                opt_tour[w-v+u+1:w+1] = tour[u+1:v+1][::-1]
                all_possible_neighbours.append(opt_tour)

                opt_tour = tour.copy()
                opt_tour[u+1:w-v+u+1] = tour[v+1:w+1][::-1]
                opt_tour[w-v+u+1:w+1] = tour[u+1:v+1]
                all_possible_neighbours.append(opt_tour)
...
def hillclimbUtil(tour, getNeighbours):
        optNeighbours = getNeighbours(tour)
        tourLengthList = [getTourLength(x) for x in optNeighbours]
        minLength = min(tourLengthList)
        return minLength,optNeighbours[tourLengthList.index(minLength)]
...
def hillClimbFull(initial_tour, getNeighbours):
    ...
    "*** YOUR CODE HERE ***"
    cur_length = getTourLength(initial_tour)
    next_length,next_tour = hillclimbUtil(initial_tour, getNeighbours)

    while next_length <= cur_length:
        tourLengthList.append(next_length)
        minTour = next_tour
        cur_length = next_length
        next_length,next_tour = hillclimbUtil(minTour, getNeighbours)
    "*** --------------  ***"
    return tourLengthList, minTour
...
def nearestNeighbourTour(initial_city):
    ...
    cur_city = initial_city

    while(explored[cur_city] == 0):
        explored[cur_city] = 1
        ...
        for i in range(1, cities + 1):
            if i != cur_city and explored[i] == 0:
                dist = getDistance(nodeDict[i],nodeDict[cur_city])
                if dist < mindist:
                    min_ind = i
                    mindist = dist
...
def eucledianTour(initial_city):
    ...
    "*** YOUR CODE HERE ***"
    # part 1
    for u in range(1, cities + 1):
        for v in range(u + 1, cities + 1):
            edgeList.append([u,v,getDistance(nodeDict[u],nodeDict[v])])
